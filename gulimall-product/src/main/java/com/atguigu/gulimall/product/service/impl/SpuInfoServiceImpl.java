...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void up(Long spuId) {
        ...
        // 获取所有的spu商品的id 然后查询这些id中那些是可以被检索的
        ...
        // 可检索的id商品规格属性集合
        List<Long> longList = attrService.selectSearchAttrs(attrIds);
        Set<Long> idSet = new HashSet<>(longList);

        //查询商品的规格属性
        List<SkuEsModel.Attrs> productAttrsList = baseAttrList.stream().filter(attr -> idSet.contains(attr.getAttrId())).map(item -> {
        ...
        // skuId 对应 是否有库存
        Map<Long, Boolean> stockMap = Maps.newHashMap();
        try {
            // 3.1 发送远程调用 库存系统查询是否有库存
            R hasStock = wareFeignService.getSkuHasStock(skuIdList);
            // 构造器受保护 所以写成内部类对象
            stockMap = hasStock.getData(new TypeReference<List<SkuHasStockTo>>(){}).stream().collect(Collectors.toMap(SkuHasStockTo::getSkuId, SkuHasStockTo::getHasStock));
        ...
        List<SkuEsModel> upProducts = skuInfos.stream().map(sku -> {
            //组装需要的数据
            ...
            skuEsModel.setAttrs(productAttrsList);
            return skuEsModel;
        }).collect(Collectors.toList());
